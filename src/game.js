...
function setup_game() {
    // Küre ve diğer geometrileri tanımlayalım
    sphere = m => lathe([0, -.5,
                         .35, .15,
                         .15, .35,
                         -.15, .35,
    ...
    // chaingun dizisine doku ekleyerek 3 boyutlu küpleri ekleyelim
    [1, 1, 1].map(x =>
    ...
    range(1).map(x => {
        chaingun.push(new Sprite(
            make_cylinder(NewVector(1, 1, 6.3)),
            mat_vector_product(matrix_rotate_yz(x * Math.PI / 8), Y_DIR), 
            null, 
            false
        ));
    });
